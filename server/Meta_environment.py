...
class MetaEnvironment(Environment):
    ...
    4 real-world agents × 3 tasks = 12 tasks total.
    ...
    def reset(self) -> MetaObservation:
        ...
        return MetaObservation(
            ...
            instructions=(
                ...
                "4 agents × 3 tasks = 12 tasks total.\n"
    ...
    def _load_context(self, task_id: str):
        if task_id == "email_triage_easy":
            ...
            return {"email": {k: v for k, v in email.items() if k != "label"}}, "easy"
        elif task_id == "email_triage_medium":
            return {"emails": [{k: v for k, v in e.items() if k not in ("priority", "category")} for e in EMAIL_MEDIUM_EMAILS]}, "medium"
        elif task_id == "email_triage_hard":
            case = random.choice(EMAIL_HARD_CASES)
            return {"email": {k: v for k, v in case.items() if k not in ("expected_elements",)}}, "hard"
        elif task_id == "code_review_easy":
            variant = random.choice(CODE_EASY_VARIANTS)
            return {"code": variant["code"], "keywords": variant["keywords"]}, "easy"
        elif task_id == "code_review_medium":
            return {"code": CODE_MEDIUM["code"]}, "medium"
        elif task_id == "code_review_hard":
            return {"code": CODE_HARD["code"]}, "hard"
        elif task_id == "data_cleaning_easy":
            return {"data": DATA_EASY["data"]}, "easy"
        elif task_id == "data_cleaning_medium":
            return {"data": DATA_MEDIUM["data"]}, "medium"
        elif task_id == "data_cleaning_hard":
            return {"data": DATA_HARD["data"]}, "hard"
        elif task_id == "content_moderation_easy":
            return {"posts": [{k: v for k, v in p.items() if k != "label"} for p in MOD_EASY]}, "easy"
        elif task_id == "content_moderation_medium":
            return {"posts": [{k: v for k, v in p.items() if k != "label"} for p in MOD_MEDIUM]}, "medium"
        elif task_id == "content_moderation_hard":
            return {"cases": [{k: v for k, v in c.items() if k not in ("label_a","label_b","reason_a","reason_b")} for c in MOD_HARD]}, "hard"
        else:
            raise ValueError(f"Unknown task_id: '{task_id}'. Use GET /tasks to see valid IDs.")

    def _grade(self, task_id, payload, context):
        if task_id == "email_triage_easy":       return grade_email_easy(payload, context)
            ...
            # Rebuild context with expected_elements for grading
            case_id = context.get("email", {}).get("id", "h1")
            full_case = next((c for c in EMAIL_HARD_CASES if c["id"] == case_id), EMAIL_HARD_CASES[0])
            return grade_email_hard(payload, {"email": full_case})
        elif task_id == "code_review_easy":      return grade_code_easy(payload, context)
    ...
    def step(self, action: MetaAction) -> MetaObservation:  # type: ignore[override]
        ...
        if not payload:
            return MetaObservation(
                ...
                feedback="❌ Empty payload. Use GET /tasks to see required payload fields.",
                score=0.0, partial_credits={}, done=True, reward=-0.1,
            )
        ...
        # Grade
        score, feedback, partial_credits = self._grade(task_id, payload, self._current_context)
