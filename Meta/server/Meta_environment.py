...
class MetaEnvironment(Environment):
    ...
    Example:
        >>> env = MetaEnvironment()
        >>> obs = env.reset()
        >>> print(obs.echoed_message)  # "Meta environment ready!"
        >>>
        >>> obs = env.step(MetaAction(message="Hello"))
    ...
    def reset(self) -> MetaObservation:
        ...
        Returns:
            MetaObservation with a ready message
        """
        self._state = State(episode_id=str(uuid4()), step_count=0)
        self._reset_count += 1

        return MetaObservation(
            echoed_message="Meta environment ready!",
            message_length=0,
            done=False,
            reward=0.0,
        )
    ...
    def step(self, action: MetaAction) -> MetaObservation:  # type: ignore[override]
        ...
        Args:
            action: MetaAction containing the message to echo

        Returns:
            MetaObservation with the echoed message and its length
        """
